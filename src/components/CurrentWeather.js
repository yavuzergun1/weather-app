  ...
  return (
    <div>
      ...
      <div className="header">
        <div className="first-container">
          ...
          <div className="data">
            <img
              className={`${
                currentData.weather[0].main === "Clear" ? "rotate" : "xrotate"
              }`}
              src={`http://openweathermap.org/img/w/${weeklyData.current.weather[0].icon}.png`}
              alt=""
            />{" "}
            {/* weather yerine weeklyData konulacak */}
            <div className="degree-container">
              <div>{currentData.weather[0].description}</div>
              <h3 className="current-deg">
        ...
        <div className="second-container">
          ...
          <div className="day">
            {moment().tz(`${weeklyData.timezone}`).format("LTS")}
          </div>
        </div>
      </div>
      <div className="wind-container">
        ...
        <i
          className={`wi wind-arrow wi-wind towards-${currentData.wind.deg}-deg`}
        ></i>
      </div>
      <div className="currentData-container">
        ...
        <div className="currentData">
          Max Temprature: {`${Math.round(currentData.main.temp_max)}`}°C
        </div>
        <div className="currentData">
          Min Temprature: {`${Math.round(currentData.main.temp_min)}`}°C
        </div>
      </div>
    </div>
  );
