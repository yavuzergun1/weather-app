    ...
    <div className='dailyWeather'>
      ...
      {(weeklyData).daily.map((day, index)=>{
       ...
       return <Link key={index} to={`/${index}`} style={{textDecoration: 'none', color:'unset'}}> 
             ...
             <div className='day-icon'><img className={`${weeklyData.daily[index].weather[0].main === 'Clear' ? 'daily-rotate' : 'daily-xrotate'}`}  src={`http://openweathermap.org/img/w/${(weeklyData).daily[index].weather[0].icon}.png`} alt="" /></div>
